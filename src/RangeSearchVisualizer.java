  ...
  public static void main(String[] args) {
    ...
    while (true) {
      ...
      if (!screenUpdated) {
        ...
        Iterable<Point2D> bruteRangePoints = brute.range(rect);
        long estimatedTime = System.nanoTime() - startTime;
        
        StdDraw.text(0.25, -0.025, "Brute: " + (float) estimatedTime / 1000000000 + " seconds");
        
        startTime = System.nanoTime();
        Iterable<Point2D> kdRangePoints = kdtree.range(rect);
        ...
        for (Point2D p : bruteRangePoints)
          p.draw();

        // draw the range search results for kd-tree in blue
        StdDraw.setPenRadius(.02);
        StdDraw.setPenColor(StdDraw.BLUE);
        for (Point2D p : kdRangePoints)
          p.draw();
